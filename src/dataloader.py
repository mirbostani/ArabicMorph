...
class ArabicMorphBaseLine(Seq2SeqDataLoader):
    def build_vocab(self):
        source_char_set, target_char_set = set(), set()
        self.nb_train = 0
        ur: List[str]
        sf: List[str]
        for ur, sf in self.read_file(self.train_file):
            self.nb_train += 1
            source_char_set.update(ur)
            target_char_set.update(sf)
        self.nb_dev = sum([1 for _ in self.read_file(self.dev_file)])
        if self.test_file is not None:
            self.nb_test = sum([1 for _ in self.read_file(self.test_file)])
        source_chars = sorted(list(source_char_set))
        target_chars = sorted(list(target_char_set))
        source = [PAD, BOS, EOS, UNK] + source_chars
        target = [PAD, BOS, EOS, UNK] + target_chars
        return source, target
    ...
    def read_file(self, file):
        with open(file, "r", encoding="utf-8") as fp:
            for line in fp.readlines():
                ...
                tokens = line.split("\t")
                if len(tokens) != 2:
                    print("WARNING: missing tokens", tokens)
                    continue
                ur, sf = tokens
                ur_list: List[str] = ur.split()
                sf_list: List[str] = sf.split()
                yield ur_list, sf_list

    def _iter_helper(self, file):
        ur: List[str] # underlying representation (morphologically tokenized word)
        sf: List[str] # surface form (uttered word found in ECAL)
        for ur, sf in self.read_file(file):
            # `source` is the indices of the list [BOS, ..., characters, ..., EOS]
            # BOS is the beginning of sentence
            # EOS is the end of sentence
            source = [self.source_c2i[BOS]]
            char: str
            for char in ur:
                source.append(self.source_c2i.get(char, UNK_IDX))
            source.append(self.source_c2i[EOS])

            # `target` is the indices of the list [BOS, ..., characters, ..., EOS]
            target = [self.target_c2i[BOS]]
            char: str
            for char in sf:
                target.append(self.target_c2i.get(char, UNK_IDX))
            target.append(self.target_c2i[EOS])

            yield source, target


class ArabicMorphBaseLineReversed(ArabicMorphBaseLine):
    def read_file(self, file):
        with open(file, "r", encoding="utf-8") as fp:
            for line in fp.readlines():
                line = line.strip()
                ...
                ur_list: List[str] = ur.split()
                sf_list: List[str] = sf.split()
                yield sf_list, ur_list # reversed
