...
class Decoder(object):
    ...
    def __call__(self, transducer, src_sentence, src_mask):
        ...
        return_values = (
            output, 
            None if self.skip_attn else attns,
            extra
        )
...
def decode_greedy_transformer(
    ...
    output_max_probs = torch.tensor([0] * bs, device=DEVICE)
    output_max_probs = output_max_probs.view(1, bs)
    output_entropy = torch.tensor([0] * bs, device=DEVICE)
    output_entropy = output_entropy.view(1, bs)
    ...
    for _ in range(max_len):
        ...
        word = torch.max(word_logprob, dim=1)[1] # 1 is max index, 0 is max val
        output = torch.cat((output, word.view(1, bs)))

        probs = torch.nn.functional.softmax(word_logprob, dim=1)
        max_prob = torch.max(probs, dim=1)[0] # max value
        output_max_probs = torch.cat((output_max_probs, max_prob.view(1, bs)))

        entropy = torch.distributions.Categorical(probs=probs).entropy()
        output_entropy = torch.cat((output_entropy, entropy.view(1, bs)))
    ...
    with torch.no_grad():
        extra = torch.stack((output_max_probs, output_entropy))
        # extra = torch.stack((output_max_probs, output_entropy, output_vowel_entropy))
