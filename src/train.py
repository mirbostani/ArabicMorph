...
class Data(util.NamedEnum):
    ...
    lematus = "lematus"
    unimorph = "unimorph"
    arabicmorphbaseline = "arabicmorphbaseline"
    arabicmorphbaselinereversed = "arabicmorphbaselinereversed"
...
class Trainer(BaseTrainer):
    ...
    def load_data(self, dataset, train, dev, test):
        ...
        if params.arch == Arch.hardmono:
            ...
            elif dataset == Data.arabicmorphbaseline:
                logger.info("Using ArabicMorphBaseLine")
                self.data = dataloader.ArabicMorphBaseLine(train, dev, test, params.shuffle)
            elif dataset == Data.arabicmorphbaselinereversed:
                logger.info("Using ArabicMorphbaseLineReversed")
                self.data = dataloader.ArabicMorphBaseLineReversed(train, dev, test, params.shuffle)
            else:
                raise ValueError
        # fmt: on
    ...
    def decode(self, mode, batch_size, write_fp, decode_fn):
        ...
        with open(f"{write_fp}.{mode}.tsv", "w") as fp:
            ...
            for src, src_mask, trg, trg_mask in tqdm(
                ...
                pred, _, extra = decode_fn(self.model, src, src_mask)
                self.evaluator.add(src, pred, trg)

                max_probs = util.unpack_extra_with_batch(pred, extra[0])
                entropy = util.unpack_extra_with_batch(pred, extra[1])
                ...
                vowel_vocab = ['A', 'E', 'I', 'O', 'U', 'a', 'i', 'u']

                for p, t, loss, entr in zip(pred, trg, losses, entropy):
                    dist = util.edit_distance(p, t)
                    p = self.data.decode_target(p)
                    t = self.data.decode_target(t)
                    entr_list = ','.join(map(str, entr))

                    # entr_max = max(entr)
                    entr_max = max(entr[1:-1])

                    vow_entr_max_v2 = 0 # not used

                    # Choose max among vowel entropies because only vowels change in arabic morph task
                    vow_entr_max_v3 = max([entr[i] for i, v in enumerate(p) if v in vowel_vocab], default=0)
                    
                    # fp.write(f'{" ".join(p)}\t{" ".join(t)}\t{loss.item()}\t{dist}\n')
                    fp.write(f'{" ".join(p)}\t{" ".join(t)}\t{loss.item()}\t{dist}\t{entr_max}\t{vow_entr_max_v2}\t{vow_entr_max_v3}\n')
                    cnt += 1
